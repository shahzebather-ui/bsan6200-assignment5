...
2. Create a **`.env`** file in the repo root (do **not** commit it):  
...
5. **Notebook (development / rubric):** open `notebooks/rag_pipeline.ipynb` and run **all cells** top-to-bottom; **save** with outputs.
...
- **Free path used here:** **Hugging Face Inference API** with a free-tier compatible **inference** model and a personal **HF_TOKEN**. No OpenAI/Gemini keys required for the submitted path.
---
